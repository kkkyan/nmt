...
from tensorflow.contrib.seq3seq.python.ops import beam_search_ops
...
class BeamSearchDecoderOutput(
    collections.namedtuple("BeamSearchDecoderOutput",
                           ("scores","predicted_ids", "parent_ids", "rnn_output"))):
  pass
...
class BeamSearchDecoder(decoder.Decoder):
  ...
  def output_size(self):
    ...
    return BeamSearchDecoderOutput(
        ...
        rnn_output = tensor_shape.TensorShape([self._beam_width]))
  ...
  def step(self, time, inputs, state, name=None):
    ...
    with ops.name_scope(name, "BeamSearchDecoderStep", (time, inputs, state)):
      ...
      if self._output_layer is not None:
        after_cell_outputs = self._output_layer(cell_outputs)

      beam_search_output, beam_search_state = _beam_search_step(
          time=time,
          logits=after_cell_outputs,
    ...
    return (beam_search_output, beam_search_state, next_inputs, finished, cell_outputs)
